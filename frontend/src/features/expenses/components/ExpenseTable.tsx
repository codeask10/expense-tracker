...
function ExpenseRow({
  ...
  // Stagger capped at 300ms so late rows don't feel laggy
  const delay = Math.min(index * 35, 300);

  return (
    <tr
      className={`
        group border-b border-gray-50 transition-colors row-animate
        ${isOptimistic ? 'bg-blue-50/40' : 'hover:bg-gray-50/70'}
      `}
    ...
    <tr>
      <td colSpan={5}>
        <div className="flex items-center justify-center gap-1.5 py-5">
          {[0, 1, 2].map((i) => (
            <span key={i} className="loader-dot" style={{ animationDelay: `${i * 160}ms` }} />
          ))}
        </div>
      </td>
    </tr>
