...
export async function fetchExpenses(
  ...
  const { data } = await apiClient.get<
    { success: boolean } & PaginatedResponse
  >("/expenses", {
    params: query,
  });
...
export async function fetchMonthlyDistribution(
  year?: number,
): Promise<MonthlyDistributionResponse> {
  const params: Record<string, number> = {};
  if (year !== undefined) params.year = year;
  const { data } = await apiClient.get<{
    success: boolean;
    data: MonthlyDistributionResponse;
  }>("/expenses/monthly-distribution", { params });
  return data.data;
}
