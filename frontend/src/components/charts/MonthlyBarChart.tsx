...
import { useMonthlyDistribution } from '../../features/expenses/hooks/useMonthlyDistribution';
import { getCategoryConfig, formatRupees } from '../../features/expenses/utils/expense.utils';
import { Card } from '../ui/Card';
import type { MonthData } from '../../features/expenses/types';
        ...
        {monthData.categories.map((cat) => {
          ...
          return (
            <div key={cat.name} className="flex items-center gap-2.5">
              ...
              <span className="font-medium text-gray-800 tabular-nums pl-2">
...
export function MonthlyBarChart() {
  ...
  // Collect unique categories across all months for the legend
  ...
  function handleBarEnter(e: React.MouseEvent<HTMLDivElement>, monthData: MonthData) {
    ...
    // Clamp so tooltip doesn't overflow left/right of container
                    ...
                    >
                      ...
                      <div
                        style={{
                          width: '80%',
                          margin: '0 auto',
                          height: hasMonthData ? BAR_HEIGHT_PX : 3,
                          display: 'flex',
                          flexDirection: 'column-reverse',
                          overflow: 'hidden',
                          borderRadius: '4px 4px 0 0',
                          cursor: hasMonthData ? 'pointer' : 'default',
                          transformOrigin: 'bottom',
                          animation: hasMonthData
                            ? `barGrow ${320 + i * 40}ms cubic-bezier(0.34,1.1,0.64,1) ${i * 40}ms both`
                            : 'none',
                          willChange: 'transform',
                          backgroundColor: hasMonthData ? 'transparent' : '#f3f4f6',
                        }}
                      ...
                      >
                        {hasMonthData &&
                          monthData.categories.map((cat) => {
                            const heightPct = (cat.amount / monthData.total) * 100;
                            const cfg = getCategoryConfig(cat.name);
                            return (
                              <div
                                key={cat.name}
